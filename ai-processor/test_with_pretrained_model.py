"""
Script para testar imagens usando o modelo pré-treinado baixado.
"""
...
if model_path.exists():
    ...
    print("Execute primeiro: python integrate_yolov8_improvements.py")

def test_images_with_model(folder_path: str):
    """
    Testa imagens usando o modelo pré-treinado.
    """
    # Criar reconhecedor (vai carregar o modelo automaticamente)
    recognizer = LicensePlateRecognizer()
    
    # Verificar se o modelo foi carregado
    if recognizer.ultra_available:
        print(f"[OK] Modelo Ultralytics carregado com sucesso!")
    else:
        print(f"[AVISO] Modelo Ultralytics nao foi carregado")
        print(f"Verifique se ULTRALYTICS_MODEL_PATH esta configurado corretamente")
    ...
    print(f"\n{'='*80}")
    print(f"TESTE COM MODELO PRE-TREINADO")
    print(f"{'='*80}")
        ...
        try:
            ...
            if results:
                ...
                for i, result in enumerate(results, 1):
                    ...
                    print(f"  Placa {i}:")
                    print(f"    Numero: {plate_num}")
                    print(f"    Tipo: {plate_type}")
    ...
    import json
    output_file = folder / "recognition_results_pretrained.json"
...
if __name__ == "__main__":
    ...
    test_images_with_model(folder_path)
